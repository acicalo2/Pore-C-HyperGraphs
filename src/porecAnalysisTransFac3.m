...
addpath C:\Pore-C\Figure7_Table1\active
addpath C:\Pore-C\DATE\01012022\
addpath C:\Pore-C\GeneExpression\GenesExpressed
addpath C:\Pore-C\DATE\01122022
addpath C:\Pore-C\DATE\01212022
addpath C:\BMatrix
addpath C:\Pore-C\edgetables
...
if type == 1 && exist('edgeTable','var') == 0
    %load('edgeTable_IR_Fibroblasts.mat')
    load('edgeTable_Fibroblast_Combined_enhancer.mat')
elseif type == 2 && exist('edgeTable_GM12878','var') == 0
    load('edgeTable_GM12878_new.mat')
elseif type == 3 && exist('edgeTable_BJ','var') == 0
    %load('BJ_edgeTable_wholegenome_12082021.mat');
    load('scBJ_edgeTable_03242022.mat');
end
toc
...
if type == 1
    ...
    load('IMR90_IR_GeneTPMgreater0.mat')
    totalGenes = IR_Genes_Expressed.Row; 
    geneTPM = IR_Genes_Expressed.NormTPM;

    load('IR_Norm_Btable.mat')
  %{
...
elseif type == 3
    load('BJ_RNA_DATABASE.mat')
    load('BJ_Norm_Btable.mat')
    totalGenes = RNA_Database.Row; 
    geneTPM = RNA_Database.NormTPM;
end
...
%     i
    ...
    if size(genes, 1)>=2
        ...
        tfIdx = sum(tfBind, 1)>=length(genes)/2; %% This line was edited to include TF that bind to the majority of genes in a cluster
